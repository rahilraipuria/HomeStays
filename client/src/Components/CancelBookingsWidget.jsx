...
const CancelBookingsWidget = ({ placeId, bookingDetails, setBookingDetails, placeTitle, placeOwner }) => {
  ...
  async function sendMailToHost(){
    ...
    try{
      const {data} = await axios.get(`http://localhost:3000/user/details/${placeOwner}`);
  ...
  const cancelBooking = async () => {
    try {
      await axios.delete(`http://localhost:3000/place/${placeId}`, { data: { checkIn, checkOut } });
      await axios.delete('http://localhost:3000/user/bookings', { data: { placeId, bookingId } });
