...
export default function BookingWidget({ place }) {
  ...
  async function sendMailToHost(){
    ...
    try{
      const {data} = await axios.get(`http://localhost:3000/user/details/${place.owner}`);
  ...
  useEffect(() => {
    ...
    const fetchBookingDetails = async () => {
      const { data } = await axios.get(`http://localhost:3000/booking/${place._id}`);
  ...
  async function bookThisPlace() {
    ...
    try {
      ...
      await axios.put("http://localhost:3000/user/bookings", {
        bookingId: data._id,
        bookedPlace: place._id,
      });
      await axios.put(`http://localhost:3000/place/${place._id}`, {
        checkIn: formattedDate(checkIn),
        checkOut: formattedDate(checkOut),
      });
